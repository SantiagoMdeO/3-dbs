...
from model import Share, SharesUpdate
...
def create_post(request: Request, post: Share = Body(...)):
...
@router.get("/", response_description="List all posts", response_model=List[Share])
...
#this works
@router.get("/id/{id}", response_description="Get a single post by ID", response_model=Share)
...
@router.get("/query/", response_description="get many posts by your query(not by _id)", response_model=Share)
def find_post(request: Request, post: SharesUpdate = Body(...)):
...
# Update a post
@router.put("/id/{id}", response_description="Update a post", response_model=Share)
def update_post(id: str, request: Request, post_update: SharesUpdate = Body(...)):
