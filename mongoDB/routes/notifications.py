...
from model import Notification, NotificationsUpdate
...
def create_post(request: Request, post: Notification = Body(...)):
...
@router.get("/", response_description="List all posts", response_model=List[Notification])
...
#this works
@router.get("/id/{id}", response_description="Get a single post by ID", response_model=Notification)
...
@router.get("/query/", response_description="get many posts by your query(not by _id)", response_model=Notification)
def find_post(request: Request, post: NotificationsUpdate = Body(...)):
...
# Update a post
@router.put("/id/{id}", response_description="Update a post", response_model=Notification)
def update_post(id: str, request: Request, post_update: NotificationsUpdate = Body(...)):
